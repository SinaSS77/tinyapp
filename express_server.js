...
app.use(
  cookieSession({
    //after installing the cookie-session to save the cookies in a secure way
    name: "session",
    keys: ["SunnyDays"],
  })
);
...
app.get("/urls/:id", (req, res) => {
  ...
  res.status(400).send("You are only allowed to delete your own URLs!");
});
...
app.post("/register", (req, res) => {
  ...
  const hashedPassword = bcrypt.hashSync(realPassword, 10);
  if (!email || !realPassword) {
    res
      .status(400)
      .send("You can not leave the password or email boxes empty!");
  }
  ...
  res
    .status(400)
    .send(
      "This email already exists. Either please log in with or choose another email address."
    );
});
...
app.post("/login", (req, res) => {
  ...
  if (!email || !password) {
    res.status(400).send("Please enter a valid email and valid password");
    return;
  }
  if (userID === null) {
    res.status(400).send("Your email is not in registered in this website!");
    return;
  }
  ...
  res
    .status(403)
    .send("Either your email address or your password is incorrect!");
});
...
app.post("/urls/:id", (req, res) => {
  const userID = req.session.user_id;
  if (!userID) {
    res.status(400).send("You may not logged in!");
    return;
  } else {
    const userURLs = urlsForUser(userID, urlDatabase);
